...
class RequestQueuePage extends Component {
  state = {
    error: false,
    isSubmitted: false,
    rawRows: null,
  ...
  componentDidMount() {
    ...
    this.props.axiosStore.instance
      .get('/rest/v1/reqQueues/')
      .then(response => {
        if (response.data.hasOwnProperty('requests_queue')) {
          // console.log(response.data);
          this.setState({ rawRows: response.data.requests_queue });
          this.setState({ tamperRow: response.data.requests_queue });
        } else {
    ...
    if (rawRows) {
      ...
      _.map(rawRows, function(item) {
        let row = {
          id: item.id.toString(),
          detail: (
            <a href={`/#/reqQueue/${item.id.toString()}/detail`}>
              <Button
                kind="secondary"
                // hasIconOnly
                renderIcon={View20}
          ...
          host: item.req_host,
          url: (
            <TextArea rows={1} cols={40} defaultValue={item.req_url} />
          ),
                ...
                <div className="bx--col-md-4 bx--col-lg-16">
                  <h1 className="landing-page__heading">
                    ...
                    <Breadcrumb
                      ...
                      <BreadcrumbItem>
                        <a href="/#">Request Queue Summary</a>
                      </BreadcrumbItem>
                    </Breadcrumb>
                  </h1>
                  <hr />
...
export default inject('sessStore', 'axiosStore')(observer(RequestQueuePage));
