...
class AnalyzedPage extends Component {
  state = {
    error: false,
    isSubmitted: false,
    rawRows: null,
    ...
    this.props.axiosStore.instance
      ...
      .then(response => {
        if (response.data.hasOwnProperty('analyzed_traffics')) {
          ...
          this.setState({ rawRows: response.data.analyzed_traffics });
          this.setState({ tamperRow: response.data.analyzed_traffics });
        } else {
    ...
    if (rawRows) {
      ...
      _.map(rawRows, function(item) {
        let row = {
          id: item.id.toString(),
          detail: (
            <a href={`/#/analyzed/${item.id.toString()}/detail`}>
              <Button
                kind="secondary"
                // hasIconOnly
                renderIcon={View20}
...
export default inject('sessStore', 'axiosStore')(observer(AnalyzedPage));
