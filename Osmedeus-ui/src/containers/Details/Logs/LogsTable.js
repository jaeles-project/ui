...
class LogsTable extends Component {
  ...
  getContent(workspace) {
    ...
    let url = `/api/logs/${workspace}`;
    this.props.axiosStore.instance
      .post(`${url}`)
      .then(response => {
        if (response.data.hasOwnProperty('commands')) {
          this.setState({ content: true });
          this.setState({ rawRows: response.data.commands });
          this.setState({
            tamperRow: response.data.commands,
          });
        } else {
          this.setState({ error: true });
        }
      ...
      _.map(rawRows, function(item, index) {
        let row = {
          ...
          result: (
            <div>
              <Button
                href={`${base_url}/../stdout${item.output_path}`}
                renderIcon={Launch20}
                small
                kind="tertiary">
                Output
              ...
              <Button
                href={`${base_url}/../stdout${item.std_path}`}
                renderIcon={Launch20}
                small
                kind="secondary">
                Std Path
