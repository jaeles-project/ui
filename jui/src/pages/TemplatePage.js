    ...
    this.props.axiosStore.instance
      ...
      .then(response => {
        if (response.data.hasOwnProperty('template')) {
          ...
          this.setState({ rawRows: response.data.template });
          this.setState({ tamperRow: response.data.template });
        } else {
    ...
    if (rawRows) {
      ...
      _.map(rawRows, function(item) {
        let row = {
          ...
          detail: (
            <a href={`/#/template/${item.name}/detail`}>
              <Button
                kind="secondary"
                // hasIconOnly
                renderIcon={View20}
          ...
          name: item.name,
          value: <Tag type="cyan">{_.join(item.values, ', ')}</Tag>,
          desc: item.desc,
          note: item.note,
        };
        realRows.push(row);
...
export default inject('sessStore', 'axiosStore')(observer(Templates));
