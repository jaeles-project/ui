...
class WorkspaceTable extends Component {
  ...
  getContent(workspace) {
    ...
    // api/summaries/get/?workspace=duckduckgo.com
    let url = `/api/summaries/get/?workspace=${workspace}`;
    this.props.axiosStore.instance
      .get(`${url}`)
      .then(response => {
        if (response.data.hasOwnProperty('summaries')) {
          this.setState({ content: true });
          this.setState({ rawRows: response.data.summaries });
          this.setState({
            tamperRow: response.data.summaries,
          });
        } else {
          this.setState({ error: true });
        }
    ...
    if (rawRows) {
      ...
      _.map(rawRows, function(item, index) {
        let row = {
          ...
          ip: item.ip_address,
          techs: (
            <WorkspaceTag data={item.technologies} color={'cyan'} />
          ),
