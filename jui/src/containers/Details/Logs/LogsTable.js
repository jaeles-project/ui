...
class LogsTable extends Component {
  ...
  getContent(workspace) {
    ...
    let url = `/api/logs/get/?workspace=${workspace}&raw=true`;
    this.props.axiosStore.instance
      .get(`${url}`)
      .then(response => {
        if (response.data.hasOwnProperty('logs')) {
          this.setState({ content: true });
          this.setState({ rawRows: response.data.logs });
          this.setState({
            tamperRow: response.data.logs,
          });
        } else {
          this.setState({ error: true });
        }
    ...
    if (rawRows) {
      ...
      _.map(rawRows, function(item, index) {
        let row = {
          ...
          // status: <Tag type="cyan">{item.status}</Tag>,
          module: item.module,
          result: ( <LogsButton output_path={item.output_path} std_path={item.std_path} base_url={base_url} />
          ),
        };
        realRows.push(row);
      });
...
export default inject('sessStore', 'axiosStore')(observer(LogsTable));
